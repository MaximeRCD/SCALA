...
trait DecoderInstances:
  ...
  given Decoder[Int] =
  Decoder.fromFunction {case Json.Num(n) if (n.isValidInt) => Option(n.toInt)
  case _ => throw new IllegalArgumentException(" not valid int")}
  /** A decoder for `String` values */
  // TODO Define a given value of type `Decoder[String]`
  given Decoder[String] =
  Decoder.fromFunction { case Json.Str(str) => Option(str)
  case _ => throw new IllegalArgumentException(" not valid string")}
  /** A decoder for `Boolean` values */
  // TODO Define a given value of type `Decoder[Boolean]`
  given Decoder[Boolean] =
  Decoder.fromFunction{case Json.Bool(b)=> Option(b)
  case _ => throw new IllegalArgumentException(" not valid bool")
  }
  /**
