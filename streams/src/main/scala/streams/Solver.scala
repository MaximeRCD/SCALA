...
trait Solver extends GameDef:
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = {
    val next = for (
      (nei_block,move) <- b.legalNeighbors) yield(nei_block,move::history)
    next.to(LazyList)
  }
  ...
  def from(initial: LazyList[(Block, List[Move])],
           explored: Set[Block]): LazyList[(Block, List[Move])] = {
    if (initial.isEmpty) LazyList[(Block, List[Move])]()
    //generate all the possible new paths
    else {
      val more = for {
        (starting_points,list_of_move)<-initial
        //next path by extending current path
        //for all possible moves
        (newblocks, updated_lists) <- newNeighborsOnly(neighborsWithHistory(starting_points,list_of_move),explored)
        if !(explored contains newblocks)
      } yield (newblocks, updated_lists)
   ...
   */
  lazy val pathsFromStart: LazyList[(Block, List[Move])] = from(LazyList((startBlock, List[Move]())), Set[Block]())
